import{_ as n,c as t,a,o as i}from"./app-NDFZUcel.js";const p="/screenshot/ds/image-6.png",r="/screenshot/ds/image-3.png",o="/screenshot/ds/image-4.png",s="/screenshot/ds/image-5.png",d="/screenshot/ds/image-7.png",l="/screenshot/ds/image-8.png",h={};function c(g,e){return i(),t("div",null,[...e[0]||(e[0]=[a('<p><a href="/pdfs/ds/%E9%93%BE%E8%A1%A8.pdf">本页PDF</a></p><h2 id="反转链表" tabindex="-1"><a class="header-anchor" href="#反转链表"><span>反转链表</span></a></h2><figure><img src="'+p+`" alt="alt text" tabindex="0" loading="lazy"><figcaption>alt text</figcaption></figure><h3 id="反转链表迭代做法" tabindex="-1"><a class="header-anchor" href="#反转链表迭代做法"><span>反转链表迭代做法</span></a></h3><p><strong>通过创建一个新的节点prev，将旧链表从前往后遍历</strong></p><pre><code>class Solution {
public:
...
</code></pre><h3 id="快乐数" tabindex="-1"><a class="header-anchor" href="#快乐数"><span>快乐数</span></a></h3><h4 id="使用快慢指针思想来做这道题" tabindex="-1"><a class="header-anchor" href="#使用快慢指针思想来做这道题"><span>（使用快慢指针思想来做这道题）</span></a></h4><figure><img src="`+r+`" alt="alt text" tabindex="0" loading="lazy"><figcaption>alt text</figcaption></figure><ul><li>情况1：一直在1中死循环</li><li>情况2：一直在某些数据中死循环，但到达不了1<br><strong>因此，我们可以使用快慢指针的思想来解题</strong></li></ul><p>代码：</p><pre><code>class Solution {
public:
    int get(int n)
    {
        int y=0,d;
...
</code></pre></li></ul><p><strong>好处：</strong></p><ol><li>不需要考虑边界条件，因为有了虚拟头节点的存在</li><li>运行速度更快</li></ol><h2 id="环形链表ii" tabindex="-1"><a class="header-anchor" href="#环形链表ii"><span>环形链表Ⅱ</span></a></h2><p><img src="`+d+'" alt="alt text" loading="lazy"><br><strong>这道题的解法非常巧妙</strong><br><strong>同样使用快慢指针，得出其路程之间的关系</strong></p><ul><li>设定两个指针，一个一次走两步，一个一次走一步（设头节点距环入口为的距离为a）</li><li>如果有环，他俩必定相遇</li><li>通过数学关系得出相遇的位置到环入口的距离等于头节点到环入口的距离 <ul><li>设环长度为x，在慢指针刚到达环入口时，快指针领先他a的距离</li><li>快指针与慢指针距离为x-a</li><li>快指针追上慢指针需要再走x-a次</li><li>慢指针在此过程中走了x-a步，相遇时的位置距离环入口为x-（x-a）=a的距离</li><li>相遇位置与头节点距环入口距离相同<br><img src="'+l+`" alt="alt text" loading="lazy"></li></ul></li><li>将其中一个指针指向头节点，让它们以相同速度走，相遇位置即环入口</li></ul><p>附上代码：</p><pre><code>class Solution {
public:
