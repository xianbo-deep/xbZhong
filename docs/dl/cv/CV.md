...
**损失函数**
$$
L(p,\mu,t^{\mu},v) = \underbrace{L_{cls}(p,\mu)}_{\text{分类损失}} + \underbrace{\lambda[\mu \ge 1]L_{loc}(t^{\mu},v)}_{\text{边界框回归损失}}
$$
其中：
...
$$
L(\{p_i\},\{t_i\}) = \underbrace{\frac{1}{N_{cls}}\sum_iL_{cls}(p_i,p_i^*)}_{\text{分类损失}} + \underbrace{\lambda\frac{1}{N_{reg}}\sum_ip_i^*L_{reg}(t_i,t_i^*)}_{\text{边界框回归损失}}
$$
...
$$
L_{\text{正}} = -2\log{(0.5)} = 1.386 \\
L_{\text{负}} = 98 \times 0.683 = 67.914 \\
