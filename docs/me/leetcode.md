...
- **单调栈**：找到当前位置左边和右边高度最高的柱子

### 滑动窗口

####  无重复字符的最长子串

**滑动窗口 + 哈希表**：维护字符最近出现的位置，有重复字符时移动左指针，否则移动右指针，同步更新最大值
...
**单调栈**：单调递减栈，找到当前位置左边和右边比他小的第一个位置，这是当前位置能扩展的最大距离
...
**哈希表 + 优先队列**：哈希表维护数字和出现次数的映射，优先队列根据出现次数进行排序，最后从优先队列弹出元素即可
...
**贪心**：维护遍历到的最小值，同时更新最大利润

####  跳跃游戏

**贪心**：维护当前能跳的最远位置，如果出现下标超过当前能跳的最远位置，说明到不了终点

####  跳跃游戏 II

**贪心**：同跳跃游戏，但当下标走到最远位置时，才更新跳跃次数
