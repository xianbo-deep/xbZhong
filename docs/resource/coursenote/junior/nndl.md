...
> 输入：训练集 $\mathcal{D} = \{x^{(n)},y^{(n)}\}_{n=1}^N$ ，验证集 $\mathcal{V}$ ，学习率 $\alpha$
>
> 1. 随机初始化 $\theta$
>
> 2. repeat
...
$$
p(y|x;w,\sigma) = \prod_{n=1}^N{N(y^{(n)};w^Tx^{(n)},\sigma^2)}
$$
...
s.t. \quad w_k^Tw^k = 1 \quad w_k w_j = 0 \quad (j = 1,\dots,k-1)
$$

- 线性判别分析

$$
\max_w J(w) = \max_w \frac{w^TS_bw}{w^TS_ww}
$$
...
$$
\frac{\partial{z_i^{(l)}}}{\partial{w_{ij}^{(l)}}} = a_{j}^{(l - 1)}
$$

所以

$$
\frac{\partial{\mathcal{L}}}{\partial{w_{ij}^{(l)}}} = \frac{\partial{\mathcal{L}}}{\partial{z_i^{(l)}}} a_{j}^{(l - 1)}
$$

这时候定义

$$
\delta_{i}^{(l)} = \frac{\partial{\mathcal{L}}}{\partial{z_i^{(l)}}}
$$

所以

$$
\frac{\partial{\mathcal{L}}}{\partial{w_{ij}^{(l)}}} = \delta_{i}^{(l)} a_{j}^{(l - 1)}
$$
...
$$
\frac{\partial{\mathcal{L}}}{\partial{b_i^{(l)}}} = \frac{\partial{\mathcal{L}}}{\partial{z_i^{(l)}}} = \delta_{i}^{(l)}
$$
...
$$
\frac{\partial{\mathcal{L}}}{\partial{z^{(l)}}} = \frac{\partial{a^{(l)}}}{\partial{z^{(l)}}}\frac{\partial{z^{(l + 1)}}}{\partial{a^{(l)}}}\frac{\partial{\mathcal{L}}}{z^{(l + 1)}}
$$

逐个看

$$
\frac{\partial{a^{(l)}}}{\partial{z^{(l)}}} = f'_l(z^{(l)})
...
可使用 Wasserstein 距离衡量不同分布之间的的远近
...
环境转移概率 $p(s_{t+1}|s_t,a_t)$ 和初始状态分布不含 $\theta$ ，所以求梯度时只剩策略项
...
$$
\begin{aligned}
\nabla_{\theta}J(\theta)&=E_{\tau \sim p_{\theta}(\tau)}[\sum_{t=1}^T{\nabla_{\theta}\log{\pi_{\theta}(a_t|s_t)}}(G(\tau_{1:t-1})+\gamma^t G(\tau_{t:T}))]  \\
&= E_{\tau \sim p_{\theta}(\tau)}[\sum_{t=1}^T{(\nabla_{\theta}\log{\pi_{\theta}(a_t|s_t)}}\gamma^t G(\tau_{t:T}))]
\end{aligned}
$$
